 ...
 * "is this domain finished enough to offer", and the honest answer for the MVP is
 * that Booking and Smart Space is the one with real content and a real UI path
 * (ARCHITECTURE §A.3). The others are listed so the layer is visible, and disabled so
 * nobody starts work the product cannot yet support.
 */

export const SUPPORTED_DOMAIN_PROFILE_KEYS: readonly string[] = ["booking_smart_space"];
