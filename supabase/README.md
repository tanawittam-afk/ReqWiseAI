...
| `20260725000015_workflow_activity_types.sql` | Slice 6A: the eight question/quality `review_activity_type` labels. Its own migration because Postgres will not let a transaction *use* an enum value it added itself |
| `20260725000016_question_and_quality_workflow.sql` | `item_workflow_state`, five workflow columns on `analysis_items` with type-keyed CHECK constraints, two workflow columns on `review_activities`, the two transition functions, `resolve_open_question()` and `update_quality_finding()`; `enforce_insert_draft()` and `guard_item_update()` extended |
| `20260725000017_blank_text_is_empty.sql` | `blank_to_null()` — Postgres `trim()` removes spaces only, so a required note made of a newline and a tab passed every "words are required" rule. All four RPCs corrected |
