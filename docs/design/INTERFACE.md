...
| Quality tab (Phase 2, 2026-09-17 plan) | A 4th workspace tab, `quality-panel.tsx`, additive to §5's Details·Evidence·Relations set — score (`qualityScore()`, `lib/analysis/workspace-view.ts`) + per-kind breakdown + the run's open findings, each linking back into the Findings tab. No single item maps to this tab directly (`?item=` deep links resolve here only for a `coverage_gap`, via `tabForType()`) |
| Coverage-gap and weakly-supported lists (Phase 3, 2026-09-17 plan) | The Quality tab's other two sections, both real data now. "Weakly supported" (`weaklySupportedItems()`) lists a requirement with no citation or `evidence_strength` under 0.5 — row only, no workflow (it names an existing requirement, not something to resolve). "Discussed but not written" lists open `coverage_gap` items — a new first-class item type, full parity with `quality_finding`'s workflow (open/acknowledged/resolved/dismissed via `update_coverage_gap()`), produced by a code-locate → segment → AI-filter pipeline (`lib/analysis/coverage-gaps.ts`) that runs as part of the same analysis, fail-open on any failure. Each gap row: displayId + title (click selects it — `tabForType("coverage_gap")` routes to `"quality"`, and the inspector's Resolution tab carries the acknowledge/resolve/dismiss controls, the same `WorkflowActions`/`WorkflowTab` components a quality finding uses) plus "Add requirement from this", identical wiring to a quality finding's own button |
